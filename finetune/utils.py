...
def eval_pc(pc_questions, pc_csv_file, log, model, tokenizer, causal_mask, kv_cache, max_generated_tokens, temperature, top_k, iteration=0, step=0, split='<|eot_id|>'):
    log.info(f"Evaluating politcal compass: iteration {iteration}, step {step}")
    answers = eval_instrs(model=model, tokenizer=tokenizer, causal_mask=causal_mask, kv_cache=kv_cache, max_generated_tokens=max_generated_tokens, temperature=temperature, top_k=top_k, instrs=pc_questions, split=split)
...
def eval_custom_prompts(custom_prompts, custom_prompts_file, log, model, tokenizer, causal_mask, kv_cache, max_generated_tokens, temperature, top_k, iteration=0, step=0, split='<|eot_id|>'):
    ...
    answers = eval_instrs(model=model, tokenizer=tokenizer, causal_mask=causal_mask, kv_cache=kv_cache, max_generated_tokens=max_generated_tokens, temperature=temperature, top_k=top_k, instrs=custom_prompts, split=split)
